...
class CreateLoyalty extends React.Component {
  constructor(props) {
    super(props);
    this.init();
    this.state = {
  ...
  init = async () => {
    try {
      if (!this.props.isSignedInForLoyalty && !this.props.loading)
        return this.props.history.push('/loyalty/list');

      const [clientResponse, statusResponse] = await Promise.all([
  ...
  addLoyalty = async () => {
    ...
    try {
      ...
      await loyaltyService.postLoyalty({
        ...data,
        status_id: +status_id,
        start_date,
        end_date,
      });
      NotificationManager.success('Success', 'Loyalty added', this.timeOut);
      this.reset();
    } catch (error) {
      console.log(error);
      NotificationManager.error(
    ...
    return (
      <div className='row'>
        <div className='col-xl-12'>
          <Panel>
            <PanelHeader>Create New Loyalty</PanelHeader>
            <PanelBody>
...
const mapStateToProps = (state, auth = state.auth) => ({
  isSignedInForLoyalty: auth.signedInForAdminRole,
  hasError: auth.hasError,
  loading: auth.loading,
});

export default connect(mapStateToProps)(withRouter(CreateLoyalty));
